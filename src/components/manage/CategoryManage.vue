<template>
        ...
        <el-table-column prop="title" label="文章标题" min-width="200">
          <template #default="scope">
            <div class="article-title-cell">
              <el-image
                v-if="scope.row.cover_image_url"
  ...
  // 假设你有一个文章API服务
  // import { getArticles, deleteArticle, publishDraftArticle } from '@/api/article';
  ...
  const fetchArticles = async () => {
    ...
    try {
      ...
      articles.value = Array(10).fill().map((_, i) => ({
        id: i + 1,
        title: `文章标题 ${i + 1}`,
        category: ['research', 'studio', 'play', 'feel', 'say'][Math.floor(Math.random() * 5)],
